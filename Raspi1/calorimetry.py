...
def calculateWater(volume_bottle,tempHotTank,tempColdTank,desiredTemp):#this is when we assume there is no water yet in the bottle (so no starting temp in bottle)
    ...
    if not tempColdTank<=desiredTemp<=tempHotTank: 
       print("Error, choose a temperature between"+tempColdTank+"°C and "+ tempHotTank+"°C" )     
    else:       #formula is derived from m_cold*C_water*T_cold+m_hot*C_water*T_hot=(m_cold+mhot)*C_water*T_desired C is constant so you can drop it 
