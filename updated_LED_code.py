import time
import glob
import requests                          # NEW: for sending data to the API
...
def select_channel(ch: int):
    bits = format(ch, '03b')
    GPIO.output(S0, int(bits[2]))
    GPIO.output(S1, int(bits[1]))
    GPIO.output(S2, int(bits[0]))
...
# ════════════════════════════════════════════════════════════════════════
# NEW: Database upload config
# ════════════════════════════════════════════════════════════════════════
BASE_URL       = "https://studev.groept.be/api/a25EE2team203"
DB_UPLOAD_INTERVAL = 5.0   # write to DB every 5 seconds — adjust as needed

def upload_to_db(water_level: int, temperature: float):
    """
    NEW: Sends the current water level (0–7) and temperature to the database
    via the API so the Java app can read it with getBottleStatus.
    URL format: /insertBottleStatus/{level}/{temperature}
    Replace 'insertBottleStatus' with your actual endpoint name.
    """
    try:
        url = f"{BASE_URL}/insertBottleStatus/{water_level}/{temperature:.2f}"  # replace endpoint name
...
def main_loop():
    ...
    last_db_time     = clock()   # NEW: track when we last uploaded to DB

    readings_done = 0
    last_temp     = None
    current_color = Color(0, 255, 0)

    # NEW: keep track of the last values so we only upload when something changed
    ...
    try:
        while True:
            ...
            # ────────────────────────────────────────────────────────────
            # NEW block 1: upload to DB on interval, but only if we have
            # a temperature reading and the values have actually changed
            # ────────────────────────────────────────────────────────────
            if now - last_db_time >= DB_UPLOAD_INTERVAL and last_temp is not None:
                ...
                if water_level != last_uploaded_level or last_temp != last_uploaded_temp:
                    ...
                    print(f"DB upload: level={water_level}, temp={last_temp:.2f}°C")
                last_db_time = now
            # ────────────────────────────────────────────────────────────
